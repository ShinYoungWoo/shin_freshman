...
if __name__ == '__main__':
    ...
    if args.function == 'pretrain':
        ...
        #     num_workers=4
        text = open(args.pretrain_corpus_path, encoding='UTF-8').read().strip()
        ...
        # TODO [part c] [part f]:
        # - Given:
        #     1. A finetuning corpus specified in args.finetune_corpu`s_path
        ...
        with open(args.outputs_path, 'w') as fout:
            ...
            for line in tqdm(open(args.eval_corpus_path)):
                x = line.split('\t')[0]
                x = x + '⁇'
                x_indices = [pretrain_dataset.stoi[s] for s in x if s in pretrain_dataset.stoi] #자꾸 키에러 떠서,,
                x = torch.tensor(x_indices, dtype=torch.long)[None, ...].to(device)
